            ...
            journeys.map(function(journey){
              return (
                <Card style={{marginBottom:15}}>
                  <CardItem >
                    <Image source={images[journey.name]["authorImage"]} />
                    <Text onPress={function(){  component.handleCardPress(journey.name)  } /* need this wrapper function to prevent inner function from being executed on render */}>
                      {journey.name}
                    </Text>
                    <Text note>{journey.publishedOn}</Text>
                  </CardItem>
                  <CardItem cardBody>
                    <Image style={{ resizeMode: 'cover' }} source={images[journey.name]["image"]} />
                    <Button transparent textStyle={{color: '#87838B'}}>{journey.starsCount} Stars</Button>
                  </CardItem>
                </Card>
              )
            })
